...
function Clients() {
  ...
  useEffect(() => {
    // Fetching the theme color for the Clients page from the backend API
    axios
      .get("http://127.0.0.1:8000/themecolors/")
      .then((response) => {
        // Finding the color for the Clients page from the API response
        const clientsColor = response.data.find(
          (theme) => theme.page_name === "clients",
        );
        if (clientsColor) {
          setThemeColor(clientsColor.color_code);
        } else {
          console.error("Clients page color not found in the API response");
        }
      })
      .catch((error) => {
